...
export default function SentMessages(props) {
    ...
    const getMessages = async () => {
        ...
        const messageData = response.data.getSentMessages;
                            ...
                            <>
                                ...
                                >
                                    <div className="flex flex-col">
                                        ...
                                        <div className="hidden sm:block">Sent to: {message.receiver}</div>
                                    </div>
                                    <ChevronUpIcon
                                        className={`${
                                ...
                                <Disclosure.Panel className="px-4 pt-4 pb-2 text-sm text-gray-500">
                                    <div className="flex flex-col">
                                        <span className="block sm:hidden">To: {message.receiver}</span>
                                        <span>{message.sentAt}</span>
                                        <span className="mt-1">{message.message}</span>
                                    </div>
                                </Disclosure.Panel>
                            </>
                        )}
