...
const mapDispatchToProps = (dispatch) => {
    return {
        setScreenshot: (screenshot) => dispatch(actionCreators.setScreenshot(screenshot))
    };
}
...
function UploadScreenshot(props) {
    ...
    function handleImageUpload(event, file) {
        const findMatching = props.screenshot.findIndex((shot) => {
            return shot.name === file.name;
        });
        if (file !== undefined && findMatching === -1 && props.screenshot.length < 1) {
            props.setScreenshot([...props.screenshot, file]);
            if (event !== null)
                event.target.value = '';
        }
    }
    ...
    return (
        <div className="sm:col-span-6">
            ...
            <div className="flex flex-row flex-wrap mt-1">
                {props.screenshot.map((shot) => {
                    return (<ScreenshotBadge fileName={shot.name} key={shot.name}/>)
                })}
            </div>
        </div>
    );
...
export default connect(mapStateToProps, mapDispatchToProps)(UploadScreenshot);