...
import {
    BellIcon,
    CogIcon, HomeIcon, MenuAlt1Icon, QuestionMarkCircleIcon,
    ShieldCheckIcon, XIcon
} from "@heroicons/react/outline";
import QAHome from "./QAHome";
import NewTicket from "./newTicket/NewTicket";
import QAClosedTickets from "./QAClosedTickets";
import QAOpenTickets from "./QAOpenTickets";
import {ChevronDownIcon, LockClosedIcon, LockOpenIcon, SearchIcon, TicketIcon} from "@heroicons/react/solid";
...
export default function SideBar() {
    ...
    function handleMainLocationChange(newLocation) {
        clearCurrent();
        setLocation(newLocation);
        const newLocationIndex = navigation.findIndex(i => i.location === newLocation);
        navigation[newLocationIndex].current = true;
    }

    function handleSecondaryLocationChange(newLocation) {
        clearCurrent();
        setLocation(newLocation);
        const newLocationIndex = secondaryNavigation.findIndex(i => i.location === newLocation);
        secondaryNavigation[newLocationIndex].current = true;
    }
        ...
        <div className="min-h-full">
                    ...
                    <nav className="mt-5 flex-1 flex flex-col divide-y divide-cyan-800 overflow-y-auto"
                        ...
                        <div className="px-2 space-y-1">
                            {navigation.map((item) => (
                                <a
                                    key={item.name}
                                    onClick={() => handleMainLocationChange(item.location)}
                                    className={classNameJoiner(
                                        item.current ? 'bg-cyan-800 text-white' : 'text-cyan-100 hover:text-white hover:bg-cyan-600',
                        ...
                        <div className="mt-6 pt-6">
                            <div className="px-2 space-y-1">
                                {secondaryNavigation.map((item) => (
                                    <a
                                        key={item.name}
                                        onClick={() => handleSecondaryLocationChange(item.location)}
                                        className={classNameJoiner(
                                            item.current ? 'bg-cyan-800 text-white' : 'text-cyan-100 hover:text-white hover:bg-cyan-600',
                                            'group flex items-center px-2 py-2 text-sm leading-6 font-medium rounded-md'
                                        )}
            ...
            <div className="lg:pl-64 flex flex-col flex-1">
                                ...
                                >
                                    <Menu.Items className="origin-top-right absolute right-0 mt-2 w-48 rounded-md shadow-lg py-1 bg-white ring-1 ring-black ring-opacity-5 focus:outline-none">
                                        <Menu.Item>
                                            {({ active }) => (
                                                <a
                                                    onClick={() => handleSecondaryLocationChange('profile')}
                                                    className={classNameJoiner(active ? 'bg-gray-100' : '', 'block px-4 py-2 text-sm text-gray-700')}
                                                >
                                                    Profile
                                                </a>
                                            )}
                                        ...
                                        <Menu.Item>
                                            {({ active }) => (
                                                <a
                                                    onClick={() => handleSecondaryLocationChange('settings')}
                                                    className={classNameJoiner(active ? 'bg-gray-100' : '', 'block px-4 py-2 text-sm text-gray-700')}
                                                >
                                                    Settings
                                                </a>
                ...
                <main className="flex-1 pb-8">
                    {location === 'home' && (<QAHome/>)}
                    {location === 'newTicket' && (<NewTicket/>)}
                    {location === 'openTickets' && (<QAOpenTickets/>)}
                    {location === 'closedTickets' && (<QAClosedTickets/>)}
                    {location === 'profile' && (<QAProfile/>)}
                    {location === 'settings' && (<QASettings/>)}
                    {location === 'help' && (<QASupport/>)}
                </main>
            </div>
        </div>
    );
